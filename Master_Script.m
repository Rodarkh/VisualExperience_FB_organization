...
for iteration=1:n_iterations
    ...
    for k=1:length(LM_RF_shape)
        Model_RFoverlap(initial_LM_dRF, initial_V1_dRF, dendriticSize, [LM_RF_shape{k},'_OS90'], [LM_RF_shape{k},'_OS0'], V1_RFshape, connectionRule, SD, survivalGain, nSamples, iteration)
        close all
    end
end
...
generate_Figure_5(initial_LM_dRF, initial_V1_dRF, dendriticSize, V1_RFshape, connectionRule, SD, survivalGain, nSamples, n_iterations)
...
for iteration=1:n_iterations
    ...
    for k=1:length(LM_RF_shape)
        for iii=1:length(eccFactors)
            Model_RFoverlap_AspectRatio_mod(initial_LM_dRF, initial_V1_dRF, dendriticSize, [LM_RF_shape{k},'_OS90'], [LM_RF_shape{k},'_OS0'], V1_RFshape, connectionRule, SD, survivalGain, nSamples, eccFactors(iii), iteration)
        end
        close all
    end
end
...
generate_Figure_6A(initial_LM_dRF, initial_V1_dRF, dendriticSize, V1_RFshape, connectionRule, SD, survivalGain, nSamples,eccFactors, n_iterations)
