function Model_RFoverlap(dRF_LM, dRF_V1,dentriticSize, LM_Group1_RFs, LM_Group2_RFs, V1_RFs, connectivityRule,borderOfEllipseFromSD,survivalGain,numberOfSamples,iterationCycleID,motherSaveFolder)
%% HELP:
% Model_RFoverlap('DRP0', 'V1', 15, 'NR_L5_OS90', 'NR_L5_OS0', 'V1', 'isoAngle',2.5,15,5000000,1,'SaveLocation')
...
parfor i=1:numberOfSamples
    ...
    % dRF center for sampled RFs of Group2
...
saveFigFolder = [motherSaveFolder, filesep, 'Model', filesep, currSettings, filesep, LM_Group1_RFs(1:(end-5))];
titleText = [currSettings, '; ',num2str(numberOfSamples/1000),'k samples'];

saveAnalFolder = [motherSaveFolder, filesep, 'Model', filesep, currSettings, filesep, LM_Group1_RFs(1:(end-5)), filesep, 'analysis'];

if ~exist(saveFigFolder)
    mkdir(saveFigFolder);
end
if ~exist(saveAnalFolder)
    mkdir(saveAnalFolder);
end
...
saveas(gcf,[saveFigFolder filesep 'simul_init_',dRF_LM,'_',dRF_V1,'_G1_',LM_Group1_RFs,'_G2_',LM_Group2_RFs,'_',V1_RFs,'_dRF_', num2str(numberOfSamples/1000),'k_',customShortText],'fig')
export_fig([saveFigFolder filesep 'simul_init_',dRF_LM,'_',dRF_V1,'_G1_',LM_Group1_RFs,'_G2_',LM_Group2_RFs,'_',V1_RFs,'_dRF_', num2str(numberOfSamples/1000),'k_',customShortText], '-png')

%% data to save per iteration to build final figure
clear rhoG1 rhoG2  rhoSurvivingG1  rhoSurvivingG2 
[~, rhoG1]=cart2pol(rotated_samples_LM_Group1(:,1),rotated_samples_LM_Group1(:,2));
[~, rhoG2]=cart2pol(rotated_samples_LM_Group2(:,1),rotated_samples_LM_Group2(:,2));
[~, rhoSurvivingG1]=cart2pol(rotated_samples_LM_Group1(logical(survival_group1),1),rotated_samples_LM_Group1(logical(survival_group1),2));
[~, rhoSurvivingG2]=cart2pol(rotated_samples_LM_Group2(logical(survival_group2),1),rotated_samples_LM_Group2(logical(survival_group2),2));

clear dRF_amp_counts_bins dRF_amp_counts_surv dRF_amp_counts_init
[dRF_amp_counts_surv(1,:),dRF_amp_counts_bins] = histcounts( [rhoSurvivingG1; rhoSurvivingG2] , 0:1:60,'normalization','probability');
[dRF_amp_counts_init(1,:),dRF_amp_counts_bins] = histcounts( [rhoG1; rhoG2], 0:1:60,'normalization','probability');

save([saveAnalFolder filesep 'data_',dRF_LM,'_',dRF_V1,'_G1_',LM_Group1_RFs,'_G2_',LM_Group2_RFs,'_',V1_RFs,'_', num2str(numberOfSamples/1000),'k_',customShortText, '.mat'],...
    'counts_G1', 'counts_G2','survivalG1', 'survivalG2','counts_V1_G1','counts_V1_G2',...
    'dRF_amp_counts_bins','dRF_amp_counts_init','dRF_amp_counts_surv',... 
    'rhoSurvivingG1','rhoSurvivingG2','rhoG1','rhoG2',...
    'binMeans_Group1_plot','binMeans_Group2_plot',... 
    'vert_data_init','vert_data_surv',...
    'survival_group1', 'survival_group2')
